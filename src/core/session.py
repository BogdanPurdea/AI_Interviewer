...
class InterviewSession:
    ...
    def process_user_input(self, user_input: str) -> str:
        ...
        # Safety Check (Input)
        if user_input.strip() and "UNSAFE" in self.safety_agent.check_safety(user_input).upper():
    ...
    def end_session(self, summary_override: str = None) -> str:
        ...
        transcript_text = "\n".join([f"{t['role']}: {t['content']}" for t in self.state.transcript])
        
        if summary_override:
             analysis_data = {"summary": summary_override, "themes": [], "sentiment_score": 0}
        else:
             analysis = self.analyst_agent.analyze_transcript(transcript_text, self.state.topic)
             analysis_data = analysis.model_dump()

        return StorageService.save_interview(self.state.topic, self.state.transcript, analysis_data)
